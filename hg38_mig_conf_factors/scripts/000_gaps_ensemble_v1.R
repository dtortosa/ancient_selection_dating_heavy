...
#Note that an AGP (A Golden Path) file is a tab-separated file used in the context of genome sequencing to describe how components of a genome assembly fit together[1]. One feature of the AGP file is that column definitions change depending on whether the line is a component line or a gap line [2].
#- **Contig**: A non-redundant sequence formed by joining, based on sequence overlap, one or more smaller sequences²[1].
#- **Scaffold (supercontig)**: A non-redundant sequence formed by joining one or more sequence contigs. No sequence overlap is required to construct the larger sequence[2].
#- **Gap**: A subregion within an object where there is no known sequence[2].
#- **Component**: A sequence used to construct a larger sequence[2].
...
#scaffold - gaps between scaffolds in chromosome assemblies (count: 470; size range: 10 - 624,000 bases). WGS assembler can use mate-pair information to order and orient the contigs and place them into larger structures called scaffolds (or supercontig).
#Telomere — telomere gaps (count: 48; all of size 10,000 bases)
#Heterochromatin — heterochromatin gaps (count: 11; size range: 20,000 - 30,000,000 bases). Heterochromatin is a tightly packed form of DNA or condensed DNA.
#Short Arm — gaps in the short arm of the chromosome (count: 5; size range: 5,000,000 - 16,990,000 bases).
...
#I have checked that these 10 first rows showed in the scheme correspond exactly with gaps in the browser of ensemble. The ensemble browser indicate that these regions are "Golden path gap - no contigs to display" or at least NO contig. Note that the databases in USCS have start coordinates as 0-based and end coordinates as 1-based (https://genome.ucsc.edu/FAQ/FAQtracks#tracks1). In contrast, coordinates in ensemble are 1-based (https://grch37.ensembl.org/Help/Faq?id=286), so we need to add one to the start of each gap in USCS table to get the same region in the ensembl browser. Indeed, when I did not sum 1 to the start in the ensemble browser, I did not get the indication that it is a golden path gap. That indication comes summing 1 to the start position showed in the scheme.
#https://www.ensembl.org/Homo_sapiens/Location/View?r=1:1-10000;db=core
#https://www.ensembl.org/Homo_sapiens/Location/View?r=1:207667-257666;db=core
#https://www.ensembl.org/Homo_sapiens/Location/View?r=1:297969-347968;db=core
#https://www.ensembl.org/Homo_sapiens/Location/View?r=1:535989-585988;db=core
#https://www.ensembl.org/Homo_sapiens/Location/View?r=1:2702782-2746290;db=core
#https://www.ensembl.org/Homo_sapiens/Location/View?r=1:12954385-13004384;db=core
#https://www.ensembl.org/Homo_sapiens/Location/View?r=1:16799164-16849163;db=core
#https://www.ensembl.org/Homo_sapiens/Location/View?r=1:29552234-29553835;db=core
#https://www.ensembl.org/Homo_sapiens/Location/View?r=1:121976460-122026459;db=core
#https://www.ensembl.org/Homo_sapiens/Location/View?r=1:122224536-122224635;db=core

#database version:
#both the scheme and the sql page from the downloaded table ("https://hgdownload.soe.ucsc.edu/goldenPath/hg38/database/gap.sql") indicate that the version is hg38 (sql files: the MySQL commands used to create the tables).

#Number of rows: The scheme indicated that the table has 829 rows.
if (nrow(raw_gaps_data) != 829) {
	stop("ERROR! FALSE! We do not have the correct number of rows")
}
...
#VERY IMPORTANT: The scheme table indicated that it is the hg19 version. The file was downloaded from the hg19 version: "https://hgdownload.soe.ucsc.edu/goldenPath/hg38/database/"
...
unique(raw_gaps_data$V5)

#'N' for gaps of known size, 'U' for gaps of unknown size
if (unique(raw_gaps_data$V6) != "N" || length(unique(raw_gaps_data$V6)) != 1) {
	stop("ERROR! FALSE! We do have gaps with unknown size")
}
...
#remember: If the relative order and orientation of the contigs on either side of the gap is known, it is a bridged gap.

#all gaps with bridge = "yes" belongs to the type scaffold.
unique_gap_types_bridged <- unique(raw_gaps_data[which(raw_gaps_data$V9 == "yes"), ]$V8)
if (unique_gap_types_bridged != "scaffold" || length(unique_gap_types_bridged) != 1) {
	stop("ERROR! FALSE! We do not have the correct gap types for bridged gaps")
}

#all gaps with bridge = "no" belongs to the type telomere, contig, clone, centromere, heterocromatin and short arm. 
unique_gap_types_non_bridged <- unique(raw_gaps_data[which(raw_gaps_data$V9 == "no"), ]$V8)
if ("scaffold" %in% unique_gap_types_non_bridged) {
	stop("ERROR! FALSE! Scaffolds have non-bridged gaps")
}
...
#chromosome of interest are the autosomal chromosomes
selected_chromosomes <- paste("chr", 1:22, sep = "")
...
if (
	...
	TRUE %in% c(c("chrY", "chrX") %in% unique(raw_gaps_data_subset$chrom))
) {
	stop("ERROR! FALSE! We do not have the correct chromosomes")
} else {
	print("Everything is ok with the chromosomes")
	print(unique(raw_gaps_data_subset$chrom))
}
...
#The scheme table says "Note: all start coordinates in our database are 0-based, not 1-based". The databases in USCS have start coordinates as 0-based and end coordinates as 1-based (https://genome.ucsc.edu/FAQ/FAQtracks#tracks1). In contrast, coordinates in ensemble are 1-based (https://grch37.ensembl.org/Help/Faq?id=286). Therefore, we need to add one to the start (not the end!) of each gap in USCS table to get coordinates in the same format than in ensemble.
#Indeed, when I did not sum 1 to the start in the ensemble browser, I did not get the indication that it is a golden path gap. That indication comes summing 1 to the start position showed in the scheme.

#check
if (FALSE %in% c(raw_gaps_data_subset$chromStart_new - 1 == raw_gaps_data_subset$chromStart)) {
	stop("ERROR! FALSE! We do not have the correct start position in 1-based coordinates")
} else {
	print("Everything is ok with the start position")
}
...
#check that the difference between chromosome start and end plus 1 (i.e., the whole length by the segment) is equal to the variable that correspond to the gap size
if (FALSE %in% c(raw_gaps_data_subset$chromEnd - raw_gaps_data_subset$chromStart_new + 1 == raw_gaps_data_subset$size)) {
	stop("ERROR! FALSE! We do not have the correct size of the gaps")
} else {
	print("Everything is ok with the size of the gaps")
}
...
####POR AQUIII
#how we can calculate the coordinates beforehand if we need the gaps for calculate coding density!!??
